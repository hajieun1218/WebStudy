...
<%--
	session : 서버에서 사용자의 일부 정보를 저장
		-> HttpSession (각 클라이언트마다 1개 배정) --> request를 이용해서 session을 얻어온다
		-> 관련된 메소드(기능) 
			1) 저장 
				setAttribute(key,값)
								 == Object
				Cookie cookie=new Cookie(key,값)
									         == String
			2) 저장된 값 읽기
				Object getAttribute("key") => 반드시 형변환(제네릭스가 존재하지 않는다)
			3) 유지 기간 설정
				default (30분) : 초(1800초)
...
<%
	Date date=new Date();
	SimpleDateFormat sdf=new SimpleDateFormat("yyyy-M-d"); // 앞에 0이 붙으면 8진법
	...
	// 1일자의 요일
	// 1년 1월 1일 ~ 2019년 12월 31일 --> 총날수
	int total=(year-1)*365
			 +(year-1)/4
			 -(year-1)/100
			 +(year-1)/400;
	
	// 2020년 2월 29일까지 --> 전 달까지
	...
	<div class="container">
		...
		<div class="row">
			...
			<table class="table table-bordered">
				...
				<%
					color="black";
					for(int i=1;i<=lastDay[month-1];i++) { //배열은  0부터 시작하니까  month-1
						if(week==0) 
							color="red";
						else if(week==6)
							color="blue";
				...
				%>
						<td height="150" class="text-left <%=back %>" valign="top"><h3><font color="<%=color %>"><%=i %></font></h3></td>
				<%
						week++;
						if(week>6) {
							week=0;
